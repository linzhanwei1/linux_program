/* Listing 37-3 */
/* daemon_SIGHUP.c 
 * Demonstrate the use of SIGHUP as a mechanism to tell a daemon to reread its configuration file and reopen its log file.
 * In the version of this code printed in the book, logOpen(), logClose(),logMessage(), and readConfigFile() were omitted for brevity.
 * The version of the code in this file is complete, and canbe compiled and run.
 */
#include <sys/stat.h>
#include <signal.h>
...
/* Write a message to the log file. Handle variable length argument lists, with an initial format string (like printf(3)), but without 
...
/* (Re) initialize from configuration file. In a real application we would of course have some daemon initialization
 * parameters in this file. In this dummy version, we simply read a single line from the file and write it to the log.
 */
...
int main(int argc, char *argv[])
{
    ...
    while(1){
        ...
        if(unslept == 0){               /* On completed interval */
            ...
            unslept = SLEEP_TIME;       /* Rest interval */
        }
    }
}