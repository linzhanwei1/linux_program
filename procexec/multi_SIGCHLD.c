/* Listing 26-5 */
/*
 * multi_SIGCHLD.c Demonstrate the use of a handler for the SIGCHLD signal, and that multiple SIGCHLD signals
 * are not queued while the signal is blocked during the execution of the handler.
 */
...
static void sigchildHandler(int sig)
{
    ...
    /* UNSAFE: This handler uses non-async-signal-safe functions (printf(), printWaitStatus(), currTime(); see Section 21.1.2) */
...
int main(int argc, char *argv[])
{
    ...
    /* Block SIGCHLD to prevent its delivery if a child terminates before the parent commences
     * the sigsuspend() loop blelow
     */
