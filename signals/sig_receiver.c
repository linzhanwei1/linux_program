/* Listing 20-7 */
/* sig_receiver.c Usage: sig_receiver [block-time]
 * Catch and report statistics on signals sent by sig_sender.c.
 * Note that although we use signal() to establish the signal handler int this program, the use of siaction()
...
int main(int argc, char *argv[])
{
     ...
     * but for the reasons described in Section 22.7 of TLPI, sigaction() is the (strongly)
     ...
     * sleep (while another process sends us signals), and then display the mask of pending signals and unblock all signals
     */
