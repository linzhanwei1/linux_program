/* Listing 59-6 */
/* is_seqnum_sv.c
 * A simple Internet stream socket server. Our service is to provide unique sequence numbers to clients.
...
#define ADDRSTRLEN (NI_MAXHOST + NI_MAXSERV + 10)
    ...
    /* Call getaddrinfo() to obtain a list of addresses that we can try binging to */
    ...
    if(rp == NULL)
        fatal("Could not bing socket to any address");
    ...
    while(1){                                           /* Handle clients iteratively */
        ...
        if(getnameinfo((const struct sockaddr *)&claddr, addrlen, host, NI_MAXHOST, service, NI_MAXSERV, 0) == 0)
        ...
        if(close(cfd) == -1)                            /* Close connetion */
            errMsg("close");
    }
}